...
void bmmultint(bignum_t num,int k,bignum_t res)
{
	int i;
	elem_t j;
	if(!k||!num[0]){
...
void bmmultbm(bignum_t a,bignum_t b,bignum_t res)
{
	int i,j;
	elem_t k;
	bignum_t tmp;
...
//调用函数有bmcmp bmmultint bmsub tryres bmmoveleft bminverse
...
inline bignum bignum::operator *(bignum& a){
	bignum ret;
	bmmultbm(num,a.num,ret.num);
	return ret;
}

inline bignum bignum::operator *(int t){
	bignum ret;
	bmmultint(num,t,ret.num);
	return ret;
}
