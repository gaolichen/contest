...
struct Rectangles{
	void getnum(string s,int &x1,int &y1,int &x2,int &y2){
		char ch;
		s2in(s,in);
		in>>ch>>x1>>ch>>y1>>ch>>ch>>ch>>x2>>ch>>y2;
	}
	bool areOverlapping(string s1, string s2){
		int x1,x2,y1,y2,xx1,xx2,yy2,yy1;
		getnum(s1,x1,y1,x2,y2);
		cout<<x1<<' '<<y1<<' '<<x2<<' '<<y2<<endl;
		getnum(s2,xx1,yy1,xx2,yy2);
		if(x1>xx2||xx1>x2
			||y1<yy2||y2>yy1)return false;
		cout<<"a"<<endl;
...
int main(){
	...
	s1="(1,5)-(5,1)";
	s2="(2,6)-(6,2)";
	if(a.areOverlapping(s1,s2))cout<<1<<endl;
	else cout<<0<<endl;
	return 0;
}