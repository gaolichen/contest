...
class MatchMaker{
public:
	vector <string> getBestMatches(vector <string> vs, string s, int n){
		VS ret;
		vector<NODE1> ans;
		vector<NODE> a;
		int i,k;
		...
		for(j=0;j<a.size();j++)
			if(j!=i&&a[j].g==a[i].d){
				int t=0;
				for(k=0;k<a[0].chose.length();k++)
					if(a[i].chose[k]==a[j].chose[k])t++;
				if(t>=n){
					NODE1 aa;
					aa.name=a[j].name;
					aa.n=t;
