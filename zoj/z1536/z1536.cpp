...
void bmaddint(bignum_t num,int k,bignum_t res) 
{
	int i;
	if(!num[0]){
		res[0]=0;
	...
	if(i>tmp[0])tmp[0]=i;
	memcpy(res,tmp,sizeof(int)*(tmp[0]+1));
}
...
void bmmutiint(bignum_t num,int k,bignum_t res)
{
	...
	if(k==1){
		memcpy(res,num,sizeof(int)*(num[0]+1));
		return;
	}
	for(i=1;i<=num[0];i++)
		res[i]=num[i]*k;
...
int bmdiveint(bignum_t num,int k,bignum_t res)
{
	int i=0,j=0,h;
	bignum_t tmp;
	if(!k)return -1;
	...
	else memcpy(res,tmp,sizeof(int)*(tmp[0]+1));
	return j;
}
...
void bminverse(bignum_t num){
	...
	for(i=1;i<=num[0];i++)
		tmp[num[0]-i+1]=num[i];
	memcpy(num+1,tmp+1,sizeof(int)*num[0]);
}

//tryres 试商，若返回值为j，则 j*b<=a<(j+1)*b，算法为二分法
//调用 bmmutiint和bmcmp

int tryres(bignum_t a,bignum_t b){
	int top,low,mid,tmp[N],i;
...
int bmdivebm(bignum_t a,bignum_t b,bignum_t res,bignum_t mod){
	...
	if(bmcmp(a,b)<0){
		res[0]=0;
		memcpy(tmp,a,sizeof(int)*(a[0]+1));
		memcpy(mod,tmp,sizeof(int)*(tmp[0]+1));
		return 1;
	}
	int i,j,k;
	bignum_t tmp1,tmp3;
	tmp[0]=b[0];
	i=a[0]-tmp[0]+1;
	memcpy(tmp+1,a+a[0]-tmp[0]+1,sizeof(int)*tmp[0]);
	...
	tmp3[0]=k-1;
	bminverse(tmp3);
	memcpy(res,tmp3,sizeof(int)*(tmp3[0]+1));
	memcpy(mod,tmp,sizeof(int)*(tmp[0]+1));
	return 1;
}
...
int tryres1(bignum_t a,bignum_t b){
	int top,low,mid,i;
	bignum_t tmp;
	top=UNIT;low=0;mid=(top+low)/2;
	while(mid>low){
...
void bmsqrt(bignum_t num,bignum_t res,bignum_t rem){
	...
	memcpy(res,tmp,sizeof(int)*(tmp[0]+1));
	memcpy(rem,tmp1,sizeof(int)*(tmp1[0]+1));
} 
...
void bmpower(int a,int b,bignum_t res){
	...
	zero*=b;
	k=zero/DEPTH;
	memset(res+1,0,sizeof(int)*k);
	zero=zero-k*DEPTH;res[k+1]=1;
	for(i=0;i<zero;i++)
		res[k+1]*=10;
	res[0]=k+1;
	if(a==1)
...
int n,s;
char mat[30][30];
bignum ans[21][21],ret[21][21];

int mx[4]={0,0,1,-1};
int my[4]={1,-1,0,0};

inline int check(int x,int y,int k){
	if(x+mx[k]<0||x+mx[k]>=n||y+my[k]<0||y+my[k]>=n)
		return 0;
	if(mat[x+mx[k]][y+my[k]]!='0')return 0;
	return 1;
}
